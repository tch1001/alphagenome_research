"""Tests for the deterministic OpenSplice selector."""
...
_MODULE_PATH = Path(__file__).with_name("select_holdout.py")
_SPEC = importlib.util.spec_from_file_location("select_holdout", _MODULE_PATH)
...
class SelectHoldoutTest(unittest.TestCase):
  ...
  def _rows(self):
    specifications = (
        ("T1A", "Exon", "sub", -4.0, 0.001, "yes"),
        ("T2A", "Intron up", "sub", -3.0, 0.002, "yes"),
        ("del3to3", "Exon", "del1", -2.0, 0.003, "yes"),
        ("T4A", "Exon", "sub", 4.0, 0.001, "yes"),
        ("del5to7", "Intron down", "del3", 3.0, 0.002, "yes"),
        ("T8A", "5'SS", "sub", 2.0, 0.003, "yes"),
        ("T9A", "Exon", "sub", 0.01, 0.9, "no"),
        ("T10A", "Intron up", "sub", -0.02, 0.8, "no"),
        ("del11to11", "Exon", "del1", 0.03, 0.7, "no"),
        ("del12to14", "Intron down", "del3", -0.04, 0.6, "no"),
        ("T15A", "5'SS", "sub", 0.001, 0.5, "no"),
    )
    rows = []
  ...
  def test_selection_is_order_and_predictor_score_independent(self):
    ...
    self.assertEqual(
        [item.candidate.variant_id for item in selection_one[:3]],
        ["GENE_e1_T1A", "GENE_e1_T2A", "GENE_e1_del3to3"],
    )
    self.assertEqual(
        [item.candidate.variant_id for item in selection_one[3:6]],
        ["GENE_e1_T4A", "GENE_e1_del5to7", "GENE_e1_T8A"],
    )
    self.assertTrue(
