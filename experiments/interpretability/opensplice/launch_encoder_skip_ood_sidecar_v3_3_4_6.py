#!/usr/bin/env python3
"""Own Gate A, preflight, START, Gate B, then load v3.3.4.6 in-process."""
...
ATTESTATION_MODULE = '_opensplice_v3_3_4_6_ood_sidecar_bootstrap_attestation'
...
def _temporary_gate_cache() -> Path:
  root = Path(tempfile.mkdtemp(prefix='alphagenome-v3.3.4.6-dry-cache.'))
...
def _start_record(
  ...
  record = {
      ...
      'prior_v3_3_4_5_controlled_stop_archive_content_binding': gate_a[
          'prior_v3_3_4_5_controlled_stop_archive_content_binding'
      ],
  }
  if set(record) != set(bootstrap.START_RECORD_KEYS) or len(record) != 39:
    raise RuntimeError('START record is not the exact 39-key schema.')
  return record
...
def _post_start_failure(error: Exception, start: dict[str, Any]) -> None:
  ...
  if (
    ...
    raise RuntimeError('POST_START failure is not the exact 25-key schema.')
...
def _publish_model_cache_pre_import_stop(
  ...
  if (
    ...
    raise RuntimeError('Launcher cache-stop RUN_COMPLETE is not exact69.')
...
def main() -> None:
  ...
  completed = subprocess.run(
      [sys.executable, str(_HERE / 'run_device_preflight_v3_3_4_6.py'), '--run'],
  ...
  runpy.run_path(
      str(_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_6.py'),
      run_name='__main__',
  )
