#!/usr/bin/env python3
"""Generate the prospective v3.3.4.4 machine freeze from frozen predecessors."""
...
import run_encoder_skip_ood_sidecar_v3_3_4_4 as runner
...
def build_freeze() -> dict[str, object]:
  base = json.loads(
      bootstrap.V3_3_4_3_FREEZE_PATH.read_text(encoding='utf-8')
  )
  if base.pop('nonpublication_terminal_contract_v3_3_4_3', None) is None:
    raise RuntimeError('Inherited v3.3.4.3 nonpublication contract is absent.')
  base.update({
  ...
  if (
      len(base) != 85 or len(base['file_sha256']) != 120
      or len(rows) != 120
  ):
    raise RuntimeError('v3.3.4.4 freeze is not exactly 85/120/120.')
  return base
