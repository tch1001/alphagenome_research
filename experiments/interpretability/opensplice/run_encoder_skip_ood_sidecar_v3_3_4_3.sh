...
if (( $# > 1 )) || { (( $# == 1 )) && [[ "$1" != '--dry-run' ]]; }; then
  echo "v3.3.4.3 accepts no production options; --dry-run is the sole option." >&2
  exit 64
fi

required_authorization=(
  V3343_AUTHORIZED_GIT_HEAD
  V3343_AUTHORIZED_FREEZE_SHA256
  V3343_AUTHORIZED_FREEZE_SIZE_BYTES
)
...
while IFS= read -r name; do
  case "$name" in
    V3343_AUTHORIZED_GIT_HEAD|V3343_AUTHORIZED_FREEZE_SHA256|V3343_AUTHORIZED_FREEZE_SIZE_BYTES) ;;
    *)
      echo "Unapproved v3.3.4.3 authorization variable: $name" >&2
      exit 64
      ;;
  esac
done < <(compgen -e | grep '^V3343_AUTHORIZED_' || true)
...
exec "$PYTHON_BIN" \
  "$SCRIPT_DIR/launch_encoder_skip_ood_sidecar_v3_3_4_3.py" "$@"
