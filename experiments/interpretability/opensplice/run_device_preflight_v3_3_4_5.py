#!/usr/bin/env python3
"""Sole append-only, JAX-only v3.3.4.5 external device preflight."""
...
_SUCCESS_PAYLOAD = b'opensplice-v3.3.4.5-renameat2-noreplace-probe-v1\n'
_COLLISION_PAYLOAD = b'opensplice-v3.3.4.5-collision-probe-v1\n'
...
def _validate_authorized_freeze() -> dict[str, Any]:
  ...
  for name, value in expected.items():
    if frozen.get(name) != value:
      raise ValueError(f'v3.3.4.5 preflight freeze mismatch: {name}.')
  return {
...
def run_preflight() -> tuple[Path, bool]:
  ...
  if set(record) != set(bootstrap.PREFLIGHT_CONTRACT['record_keys']) or (
    ...
    raise RuntimeError('Preflight record is not the exact 22-key schema.')
