#!/usr/bin/env python3
"""Own Gate A, preflight, START, Gate B, then load v3.3.4.3 in-process."""
...
ATTESTATION_MODULE = '_opensplice_v3_3_4_3_ood_sidecar_bootstrap_attestation'
...
def _temporary_gate_cache() -> Path:
  root = Path(tempfile.mkdtemp(prefix='alphagenome-v3.3.4.3-dry-cache.'))
...
def main() -> None:
  ...
  completed = subprocess.run(
      [sys.executable, str(_HERE / 'run_device_preflight_v3_3_4_3.py'), '--run'],
  ...
  runpy.run_path(
      str(_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_3.py'),
      run_name='__main__',
  )
