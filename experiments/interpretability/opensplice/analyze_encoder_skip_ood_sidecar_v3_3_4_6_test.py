#!/usr/bin/env python3
"""CPU-only tests for the standalone v3.3.4.6 structural analyzer."""
...
_ANALYZER_PATH = _HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_6.py'
...
_V3345_ARCHIVE_BINDING = bootstrap.canonical_content_binding(
...
def _dispatch_event(
  ...
  value: dict[str, object] = {
      'schema_version': 'v3.3.4.6-dispatch-event-v1',
...
class StructuralResultAndAppendOnlyTest(unittest.TestCase):
  ...
  def test_structural_result_draft_has_literal_28_keys_and_no_science(self):
  ...
  def test_shell_accepts_only_literal_acknowledgement(self):
    shell = (_HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_6.sh').read_text(encoding='utf-8')
...
class ConsumedPrefixAndLifecycleTest(unittest.TestCase):
  ...
  def test_v3344_consumed_prefix_live_archive_and_canonical_binding(self):
    ...
    bootstrap_path = (
        _HERE / 'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6.py'
    )
  ...
  def test_embedded_prefix_requires_exact_object_and_canonical_binding(self):
    prefix, binding = _consumed_prefix()
    v3344, v3344_binding = _consumed_v3344_prefix()
    v3345_archive = {'exact_controlled_stop_archive': True}
    v3345_binding = analyzer._content_binding(v3345_archive)  # pylint: disable=protected-access
    frozen = {
        'prior_v3_3_4_3_consumed_preflight_prefix': prefix,
        'prior_v3_3_4_4_consumed_preflight_prefix': v3344,
        'prior_v3_3_4_5_controlled_stop_archive': v3345_archive,
  ...
  def test_analysis_start_result_and_failure_have_revised_exact_schemas(self):
    ...
    self.assertEqual(len(started), 19)
    self.assertEqual(
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      self.assertEqual(len(failure), 20)
      self.assertEqual(
          set(failure), {
...
class PublicationAmendmentTest(unittest.TestCase):
  ...
  def test_v3345_atomic_probe_live_bytes_and_names_are_exact(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      root.mkdir(mode=0o700)
      external_pid = 12345
      final = root / 'atomic_publication_probe_v3_3_4_6.txt'
      collision = (
          root / f'.v3345.tmp.{external_pid}.000001.{"a" * 32}'
      )
      final.write_bytes(
          b'opensplice-v3.3.4.6-renameat2-noreplace-probe-v1\n'
      )
      collision.write_bytes(b'opensplice-v3.3.4.6-collision-probe-v1\n')
      final.chmod(0o400)
  ...
  def test_publication_contract_is_literal_and_exact(self):
    ...
    preflight_source = (
        _HERE / 'run_device_preflight_v3_3_4_6.py'
  ...
  def test_publisher_delegates_to_shared_helper_and_checks_live_inode(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      with mock.patch.dict(
          sys.modules,
          {'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6': fake},
      ):
...
class NonpublicationTerminalAmendmentTest(unittest.TestCase):
  ...
  def test_exact_63_key_contract_is_literal(self):
    self.assertEqual(len(analyzer.NONPUBLICATION_TERMINAL_KEYS), 63)
    self.assertEqual(len(set(analyzer.NONPUBLICATION_TERMINAL_KEYS)), 63)
    self.assertIn(
  ...
  def test_runner_identity_literals_match_committed_protocol(self):
    runner = ast.parse(
        (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_6.py').read_text(
            encoding='utf-8'
        )
    )
    values = {}
    ...
    preflight = ast.parse(
        (_HERE / 'run_device_preflight_v3_3_4_6.py').read_text(
            encoding='utf-8'
        )
    )
    preflight_versions = [
  ...
  def test_runner_diagnostic_classifier_is_operation_typed(self):
    runner_path = _HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_6.py'
  ...
  def test_machine_extracted_serializer_keysets_match_analyzer(self):
    ...
    launcher = ast.parse(
        (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_6.py').read_text(
            encoding='utf-8'
        )
    )
    runner = ast.parse(
    ...
    self.assertEqual(
        assigned_dict_keys(function(preflight, 'run_preflight'), 'record', 23),
    ...
    self.assertEqual(
        assigned_dict_keys(
            function(launcher, '_post_start_failure'), 'record', 25
        ),
        analyzer._POST_START_FAILURE_KEYS,  # pylint: disable=protected-access
    )
    self.assertEqual(
        assigned_dict_keys(
              function(runner, '_write_common_terminal'), 'record', 69
        ),
        analyzer._RUN_COMPLETE_KEYS,  # pylint: disable=protected-access
    )
    self.assertEqual(
        assigned_dict_keys(
              function(runner, '_write_terminal_failure'), 'record', 36
        ),
        analyzer.TERMINAL_FAILURE_KEYS,
    )
    self.assertEqual(
        assigned_dict_keys(
            function(runner, '_write_nonpublication_terminal'), 'record', 63
        ),
        analyzer.NONPUBLICATION_TERMINAL_KEYS,
    )
    self.assertEqual(len(analyzer._FREEZE_KEYS), 88)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._V3345_SOURCE_PATHS), 12)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._ANALYSIS_STARTED_KEYS), 19)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._ANALYSIS_KEYS), 28)  # pylint: disable=protected-access
  ...
  def test_nonpublication_result_is_exact_structural_only_outcome(self):
    ...
    self.assertEqual(
        result['analysis_version'], 'v3.3.4.6-structural-analyzer-v1'
    )
