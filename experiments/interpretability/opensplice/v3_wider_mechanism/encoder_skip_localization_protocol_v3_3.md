...
### 2.3 Upstream source and generated-protobuf provenance

The sibling upstream AlphaGenome checkout contributes 26 Python modules to
this experiment. Gate 0 freezes every canonical relative path, SHA-256 and
byte size, with this exact prospective source-status split:

- 22 modules are Git-tracked and must be byte-identical to the pinned upstream
  HEAD with an empty tracked diff; and
- four are intentionally ignored generated-code exceptions at the canonical
  paths for `alphagenome.protos.dna_model_pb2`,
  `alphagenome.protos.dna_model_service_pb2`,
  `alphagenome.protos.dna_model_service_pb2_grpc` and
  `alphagenome.protos.tensor_pb2`.

Each exception must be a regular non-symlink file, absent from `git ls-files`,
ignored by the pinned `src/alphagenome/.gitignore`, and byte-exact before the
first model import. Their frozen bindings are:

| Generated module | SHA-256 | Bytes |
|---|---|---:|
| `dna_model_pb2.py` | `d97564536e77ec09bdf144ba1204d4e08f79095fb9ed6c0cba7b065dc6f252ee` | 16,279 |
| `dna_model_service_pb2.py` | `062ed92fd13a0757c3c029275edd698c2327ea56be4a5e074a28d91fa0dabd6a` | 10,114 |
| `dna_model_service_pb2_grpc.py` | `db3816f2f0ccfe08d0e6bec0771079bbca6cfc399bd58e79e928cb1264bdaeb3` | 16,361 |
| `tensor_pb2.py` | `dea7a5207e82601b6763e95ee4b69356345e95bc2de91632928d6161f873cdb8` | 3,155 |

The machine-readable freeze also binds the tracked source protos and nominal
recipe inputs exactly:

| Tracked input | SHA-256 | Bytes |
|---|---|---:|
| `src/alphagenome/protos/dna_model.proto` | `d19a7208ec34953ca021efbff32516f1aa277f0477276f7699d9567fd616329a` | 15,103 |
| `src/alphagenome/protos/dna_model_service.proto` | `bdcfea14fb629fb4b6e9304909023a872267a69e5f0e3a23bdb9a769f26dc317` | 8,472 |
| `src/alphagenome/protos/tensor.proto` | `07779023b2868377cbfc3c2ce96cd266ae425a0a1116aea755691c263d6238f7` | 2,856 |
| `hatch_build.py` | `7e517361e85e8f85ca0c5c8497b9281234f54215475e0e86a7695cdab7945d5b` | 1,739 |
| `pyproject.toml` | `e4b06b106b2a4836d1b94d0fb97128ec23a2aeb0b7ebf043aed302a321075777` | 3,073 |
| `src/alphagenome/.gitignore` | `7405614ddb7a97b8acdf99e14ba9f4adf07f2318480c733733da03f51cd704ec` | 358 |

Validate the embedded protobuf Python 5.27.2 headers in the three `pb2` files,
the gRPC 1.67.1 header in the `pb2_grpc` file, and live protobuf 7.35.1/gRPC
1.83.0; record that `grpcio-tools` is absent. The historical generator binary,
version and argv are not attested. The hatch hook is only the nominal source
recipe, the current generated bytes are preserved rather than regenerated,
and no reproducible-generation claim is permitted. Bootstrap, START, import
inventories and the offline analyzer must cross-bind this exact 22+4 split.
