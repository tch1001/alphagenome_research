#!/usr/bin/env python3
"""CPU-only contract tests for the v3.3.4.4 external preflight."""
...
import launch_encoder_skip_ood_sidecar_v3_3_4_4 as launcher  # pylint: disable=g-import-not-at-top
import run_device_preflight_v3_3_4_4 as preflight  # pylint: disable=g-import-not-at-top
...
def _environment(root: Path, role: str = 'external_preflight') -> dict[str, str]:
  return {
      ...
      'ALPHAGENOME_V3_3_4_4_CACHE_ROLE': role,
      'ALPHAGENOME_V3_3_4_4_CACHE_ROOT': str(root.resolve()),
      'TRITON_CACHE_DIR': str((root / 'triton').resolve()),
      'XDG_CACHE_HOME': str((root / 'xdg').resolve()),
  }


class DevicePreflightV3344Test(unittest.TestCase):
  ...
  def test_three_way_preflight_version_proof_rejects_each_drift(self):
    ...
    wrong_freeze['preflight_script_version'] = 'v3.3.4.4'
    with self.assertRaisesRegex(ValueError, 'three-way proof'):
      module.validate_preflight_version_contract(wrong_freeze)
    with mock.patch.object(module, 'PREFLIGHT_SCRIPT_VERSION', 'v3.3.4.4'):
      with self.assertRaisesRegex(ValueError, 'three-way proof'):
        module.validate_preflight_version_contract(frozen)

    with tempfile.TemporaryDirectory() as directory:
      temporary = Path(directory)
      producer = temporary / 'run_device_preflight_v3_3_4_4.py'
      producer.write_text(
          "PREFLIGHT_SCRIPT_VERSION = 'v3.3.4.4'\n", encoding='utf-8'
      )
      producer_frozen = {
          'preflight_script_version': module.PREFLIGHT_SCRIPT_VERSION,
          'file_sha256': {
      ...
      with self.subTest(stage=stage), tempfile.TemporaryDirectory() as directory:
        with self._roots(Path(directory)) as (external, _):
          ...
          if stage == 'parent_open':
            ...
            nonce = 'a' * 32
            (external / f'.v3344.tmp.{os.getpid()}.000000.{nonce}').write_bytes(b'old')
        ...
        with self.subTest(location=location, kind=kind), tempfile.TemporaryDirectory() as directory:
          with self._roots(Path(directory)) as (external, _):
            ...
            if location == 'final':
              ...
              nonce = 'a' * 32
              blocked = external / (
                  f'.v3344.tmp.{os.getpid()}.000000.{nonce}'
              )
  ...
  def test_parent_swap_between_require_and_open_creates_no_entry(self):
    with tempfile.TemporaryDirectory() as directory, self._roots(
      ...
      self.assertFalse(any(path.name.startswith('.v3344.tmp.') for path in external.iterdir()))
      self.assertFalse(any(path.name.startswith('.v3344.tmp.') for path in old.iterdir()))
  ...
  def test_launcher_dry_cache_is_nonproduction_and_removed(self):
    ...
    try:
      self.assertTrue(root.name.startswith('alphagenome-v3.3.4.4-dry-cache.'))
  ...
  def test_revised_serializers_all_bind_consumed_prefix_and_content(self):
    ...
    keysets = (
        ...
        module.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_3['keys'],
    )
    self.assertEqual([len(value) for value in keysets], [20, 36, 22, 66, 33, 60])
    for keyset in keysets:
      self.assertTrue(required <= set(keyset))

  def test_freeze_generator_is_deterministic_exact_85_120_120(self):
    generator = importlib.import_module(
        'generate_encoder_skip_ood_sidecar_v3_3_4_4_freeze'
    )
    ...
    self.assertIn('nonpublication_terminal_contract_v3_3_4_4', first)
    self.assertNotIn('nonpublication_terminal_contract_v3_3_4_3', first)
    predecessor = json.loads(
        preflight.bootstrap.V3_3_4_3_FREEZE_PATH.read_text(encoding='utf-8')
    )
  ...
  def test_source_contains_no_forbidden_publication_or_science(self):
    paths = [
        _HERE / name for name in (
            'analyze_encoder_skip_ood_sidecar_v3_3_4_4.py',
            'analyze_encoder_skip_ood_sidecar_v3_3_4_4.sh',
            'analyze_encoder_skip_ood_sidecar_v3_3_4_4_test.py',
            'generate_encoder_skip_ood_sidecar_v3_3_4_4_freeze.py',
            'launch_encoder_skip_ood_sidecar_v3_3_4_4.py',
            'run_device_preflight_v3_3_4_4.py',
            'run_device_preflight_v3_3_4_4_test.py',
            'run_encoder_skip_ood_sidecar_v3_3_4_4.py',
            'run_encoder_skip_ood_sidecar_v3_3_4_4.sh',
            'run_encoder_skip_ood_sidecar_v3_3_4_4_test.py',
            'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_4.py',
        )
    ]
    paths.append(
