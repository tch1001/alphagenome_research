#!/usr/bin/env python3
"""Deterministically selects the frozen OpenSplice circuit benchmark.
...
SELECTION_VERSION = "opensplice-circuit-v1"
NEGATIVE_PER_EXON = 3
POSITIVE_PER_EXON = 3
NEUTRAL_PER_EXON = 4
...
def select_for_exon(candidates: Sequence[Candidate]) -> tuple[Selection, ...]:
  ...
  negatives = sorted(
      (candidate for candidate in effects if candidate.delta_logit < 0),
      key=lambda candidate: (candidate.delta_logit, candidate.variant_id),
  )[:NEGATIVE_PER_EXON]
  positives = sorted(
      (candidate for candidate in effects if candidate.delta_logit > 0),
      key=lambda candidate: (-candidate.delta_logit, candidate.variant_id),
  )[:POSITIVE_PER_EXON]
  if len(negatives) != NEGATIVE_PER_EXON:
    raise ValueError(f"{exon_id}: fewer than three significant negative variants")
  if len(positives) != POSITIVE_PER_EXON:
    raise ValueError(f"{exon_id}: fewer than three significant positive variants")
  ...
  if len(neutral_pool) < NEUTRAL_PER_EXON:
    raise ValueError(f"{exon_id}: fewer than four nonsignificant neutral variants")

  selections = [
      Selection(candidate, "negative_effect", rank)
      for rank, candidate in enumerate(negatives, start=1)
  ]
  selections.extend(
      Selection(candidate, "positive_effect", rank)
      for rank, candidate in enumerate(positives, start=1)
  )

  match_targets = sorted(
      (*negatives, *positives),
