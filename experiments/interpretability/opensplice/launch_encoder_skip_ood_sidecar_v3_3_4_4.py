#!/usr/bin/env python3
"""Own Gate A, preflight, START, Gate B, then load v3.3.4.4 in-process."""
...
ATTESTATION_MODULE = '_opensplice_v3_3_4_4_ood_sidecar_bootstrap_attestation'
...
def _temporary_gate_cache() -> Path:
  root = Path(tempfile.mkdtemp(prefix='alphagenome-v3.3.4.4-dry-cache.'))
...
def _start_record(
  ...
  if set(record) != set(bootstrap.START_RECORD_KEYS) or len(record) != 36:
    raise RuntimeError('START record is not the exact 36-key schema.')
  return record
...
def _post_start_failure(error: Exception, start: dict[str, Any]) -> None:
  ...
  if (
    ...
    raise RuntimeError('POST_START failure is not the exact 22-key schema.')
...
def main() -> None:
  ...
  completed = subprocess.run(
      [sys.executable, str(_HERE / 'run_device_preflight_v3_3_4_4.py'), '--run'],
  ...
  runpy.run_path(
      str(_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_4.py'),
      run_name='__main__',
  )
