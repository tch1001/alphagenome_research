#!/usr/bin/env python3
"""CPU-only tests for the standalone v3.3.4.4 structural analyzer."""
...
_ANALYZER_PATH = _HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_4.py'
...
def _dispatch_event(
  ...
  value: dict[str, object] = {
      'schema_version': 'v3.3.4.4-dispatch-event-v1',
...
def _terminal(
  ...
  consumed, consumed_binding = _consumed_prefix()
  ...
  node.update({
      ...
      'prior_v3_3_4_3_consumed_preflight_prefix': consumed,
...
class StructuralResultAndAppendOnlyTest(unittest.TestCase):
  ...
  def test_structural_result_draft_has_literal_25_keys_and_no_science(self):
    result = analyzer._result_v3344(  # pylint: disable=protected-access
  ...
  def test_shell_accepts_only_literal_acknowledgement(self):
    shell = (_HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_4.sh').read_text(encoding='utf-8')
    self.assertIn('--acknowledge-structural-only-v3-3-4-4', shell)
...
class ConsumedPrefixAndLifecycleTest(unittest.TestCase):
  ...
  def test_consumed_prefix_live_tree_sources_and_canonical_bindings(self):
    ...
    spec = importlib.util.spec_from_file_location(
        '_v3344_bootstrap_consumed_prefix', bootstrap_path
    )
  ...
  def test_consumed_prefix_gate_precedes_run_start_and_any_raw_read(self):
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      with (
          mock.patch.object(
              analyzer, '_validate_freeze_v3344', side_effect=provenance_gate
          ),
          mock.patch.object(
              analyzer, '_validate_start_v3344', side_effect=start_read
          ),
  ...
  def test_analysis_start_result_and_failure_have_revised_exact_schemas(self):
    ...
    self.assertEqual(
        ...
        '--acknowledge-structural-only-v3-3-4-4',
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      self.assertEqual(len(failure), 17)
      self.assertEqual(
          set(failure), {
...
class PublicationAmendmentTest(unittest.TestCase):
  ...
  def _publication_terminal_fixture(
    ...
    temp_relative = (
        'compiler/eight_row/'
        f'.v3344.tmp.{runner_pid}.000000.{"a" * 32}'
    )
  ...
  def test_publication_contract_is_literal_and_exact(self):
    ...
    preflight_source = (
        _HERE / 'run_device_preflight_v3_3_4_4.py'
  ...
  def test_output_state_represents_all_publication_failure_classes(self):
    for case in ('temporary', 'uncertain', 'preexisting'):
      with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
        ...
        audit = _publication_audit()
        if case == 'temporary':
          path = output / f'.v3344.tmp.12.000001.{"a" * 32}'
        else:
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      def publish_bytes(role, relative, payload, *, artifact_role):
        ...
        return {
            ...
            'temp_basename': f'.v3344.tmp.{__import__("os").getpid()}.000000.{nonce}',
      ...
      with mock.patch.dict(
          sys.modules,
          {'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_4': fake},
      ):
...
class NonpublicationTerminalAmendmentTest(unittest.TestCase):
  ...
  def test_exact_60_key_contract_is_literal(self):
    self.assertEqual(len(analyzer.NONPUBLICATION_TERMINAL_KEYS), 60)
    self.assertEqual(len(set(analyzer.NONPUBLICATION_TERMINAL_KEYS)), 60)
    self.assertIn(
    ...
    self.assertEqual(
        ...
        bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_3,
    )
    self.assertEqual(
  ...
  def test_runner_identity_literals_match_committed_protocol(self):
    runner = ast.parse(
        (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_4.py').read_text(
            encoding='utf-8'
        )
    )
    values = {}
    ...
    preflight = ast.parse(
        (_HERE / 'run_device_preflight_v3_3_4_4.py').read_text(
            encoding='utf-8'
        )
    )
    preflight_versions = [
  ...
  def test_runner_diagnostic_classifier_is_operation_typed(self):
    runner_path = _HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_4.py'
  ...
  def test_machine_extracted_serializer_keysets_match_analyzer(self):
    ...
    launcher = ast.parse(
        (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_4.py').read_text(
            encoding='utf-8'
        )
    )
    runner = ast.parse(
    ...
    self.assertEqual(
        assigned_dict_keys(function(preflight, 'run_preflight'), 'record', 20),
    ...
    self.assertEqual(
        assigned_dict_keys(
            function(launcher, '_post_start_failure'), 'record', 22
        ),
        analyzer._POST_START_FAILURE_KEYS,  # pylint: disable=protected-access
    )
    self.assertEqual(
        assigned_dict_keys(
              function(runner, '_write_common_terminal'), 'record', 66
        ),
        analyzer._RUN_COMPLETE_KEYS,  # pylint: disable=protected-access
    )
    self.assertEqual(
        assigned_dict_keys(
              function(runner, '_write_terminal_failure'), 'record', 33
        ),
        analyzer.TERMINAL_FAILURE_KEYS,
    )
    self.assertEqual(
        assigned_dict_keys(
            function(runner, '_write_nonpublication_terminal'), 'record', 60
        ),
        analyzer.NONPUBLICATION_TERMINAL_KEYS,
    )
    self.assertEqual(len(analyzer._FREEZE_KEYS), 85)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._V3344_SOURCE_PATHS), 12)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._ANALYSIS_STARTED_KEYS), 16)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._ANALYSIS_KEYS), 25)  # pylint: disable=protected-access
  ...
  def test_nonpublication_result_is_exact_structural_only_outcome(self):
    ...
    self.assertEqual(
        result['analysis_version'], 'v3.3.4.4-structural-analyzer-v1'
    )
  ...
  def test_analyze_routes_nonpublication_without_raw_access(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      with (
          mock.patch.object(
              analyzer, '_validate_freeze_v3344',
              return_value=({}, 'b' * 64, {'prior': 333}, {}, {}, {'prior': 331}),
          ),
          mock.patch.object(analyzer, '_validate_start_v3344', return_value=start),
          mock.patch.object(
