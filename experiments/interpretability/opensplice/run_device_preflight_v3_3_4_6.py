#!/usr/bin/env python3
"""Sole append-only, JAX-only v3.3.4.6 external device preflight."""
...
_SUCCESS_PAYLOAD = b'opensplice-v3.3.4.6-renameat2-noreplace-probe-v1\n'
_COLLISION_PAYLOAD = b'opensplice-v3.3.4.6-collision-probe-v1\n'
...
def _validate_authorized_freeze() -> dict[str, Any]:
  ...
  prior_v3_3_4_5_binding = bootstrap.canonical_content_binding(
      prior_v3_3_4_5_archive
  )
  expected = {
  ...
  for name, value in expected.items():
    if frozen.get(name) != value:
      raise ValueError(f'v3.3.4.6 preflight freeze mismatch: {name}.')
  if (
      frozen.get('prior_v3_3_4_5_controlled_stop_archive')
      != prior_v3_3_4_5_archive
      or frozen.get(
          'prior_v3_3_4_5_controlled_stop_archive_content_binding'
      ) != prior_v3_3_4_5_binding
  ):
    raise ValueError('v3.3.4.5 controlled-stop archive binding changed.')
  return {
...
def run_preflight() -> tuple[Path, bool]:
  ...
  record = {
      ...
      'prior_v3_3_4_4_consumed_preflight_prefix': freeze[
          'prior_v3_3_4_4_consumed_preflight_prefix'
      ],
      'prior_v3_3_4_4_consumed_preflight_prefix_content_binding': freeze[
  ...
  if set(record) != set(bootstrap.PREFLIGHT_CONTRACT['record_keys']) or (
      len(record) != 23
  ):
    raise RuntimeError('Preflight record is not the exact 23-key schema.')
