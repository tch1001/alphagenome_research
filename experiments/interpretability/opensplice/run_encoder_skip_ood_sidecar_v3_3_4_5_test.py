#!/usr/bin/env python3
"""CPU-only contract tests for the v3.3.4.5 OOD sidecar runner."""
...
import run_encoder_skip_ood_sidecar_v3_3_4_5 as runner  # pylint: disable=g-import-not-at-top
...
class EncoderSkipOodSidecarV334Test(unittest.TestCase):
  ...
  def test_extraction_nonpublication_terminal_has_exact_six_file_archive(self):
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      with self._publication_root(str(root)), mock.patch.object(
        ...
        self.assertEqual(len(terminal), 62)
      self.assertEqual(terminal['preterminal_tree_binding']['file_count'], 5)
      self.assertEqual(
          {path.relative_to(root).as_posix() for path in root.rglob('*')
           if path.is_file()},
          set(runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5[
  ...
  def test_postcompile_orchestration_reaches_all_five_exact_stages(self):
    stages = tuple(
        runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5[
            'failure_stages'
        ]
    )
    for stage in stages:
  ...
  def test_nonpublication_same_object_rejects_missing_and_extra_keys(self):
    stages = runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5[
        'failure_stages'
    ]
    valid = _same_object()
  ...
  def test_loaded_scientific_module_contract_has_exact_root_roles(self):
    ...
    self.assertTrue({
        'v3_3_4_5_runner', 'v3_3_4_5_launcher',
  ...
  def test_wrapper_and_launcher_freeze_lifecycle(self):
    shell = (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_5.sh').read_text()
    launcher = (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_5.py').read_text()
