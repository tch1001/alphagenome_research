#!/usr/bin/env python3
"""One-shot development-only OpenSplice v3.3.4.5 OOD sidecar runner."""
...
_PREIMPORT_ATTESTATION_MODULE = (
    '_opensplice_v3_3_4_5_ood_sidecar_bootstrap_attestation'
)
if (
    __name__ == '__main__'
  ...
  raise RuntimeError(
      'Direct v3.3.4.5 execution is forbidden before pre-import bootstrap; '
      'use launch_encoder_skip_ood_sidecar_v3_3_4_5.py.'
  )
...
def _launcher_record() -> Mapping[str, Any]:
  ...
  if not isinstance(record, Mapping):
    raise RuntimeError('Missing same-process v3.3.4.5 launcher attestation.')
  return record
...
def sidecar_execution_order(
  ...
  if tuple(case.order for case in cases) != RECIPIENT_ORDERS:
    raise ValueError('v3.3.4.5 requires development recipient orders 0--19.')
  result = tuple(
      (order, anchor_id)
  ...
  if len(result) != EXPECTED_RECORD_COUNT or len(set(result)) != len(result):
    raise ValueError('v3.3.4.5 sidecar order is incomplete or duplicated.')
  return result
...
def validate_ood_sidecar_anchor(
  ...
  """Applies the sole v3.3.4.5 repair while retaining all stronger gates."""
  if anchor_id not in ANCHOR_IDS:
    raise ValueError('v3.3.4.5 received an unfrozen OOD anchor.')
...
def consume_bootstrap_attestation() -> dict[str, Any]:
  ...
  if not isinstance(record, dict):
    raise RuntimeError(
        'Direct v3.3.4.5 runner invocation is forbidden; use its launcher.'
    )
  if record.get('pid') != os.getpid():
    raise RuntimeError('v3.3.4.5 bootstrap attestation came from another process.')
  required = {
...
def import_provenance(
  ...
  return {
      'schema_version': 'v3.3.4.5-import-provenance-v1',
      'phase': phase,
      ...
      'v3_3_4_5_sidecar_sources': sidecar_sources,
...
def loaded_scientific_modules() -> list[dict[str, Any]]:
  ...
  normalized_locked = {
      (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_5.py').resolve(): (
          'v3_3_4_5_runner'
      ),
      (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_5.py').resolve(): (
          'v3_3_4_5_launcher'
      ),
      (_HERE / 'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_5.py').resolve(): (
          'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_5'
      ),
...
def _route_diagnostic_failure(
  ...
  if triggering_reason not in bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5[
...
def _partial_same_object_attestation(
  ...
  if stage not in bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5[
      'failure_stages'
  ][:3]:
...
def _counted_apply(
  ...
  event_common = {
      'schema_version': 'v3.3.4.5-dispatch-journal-v1',
      'attempt_id': ATTEMPT_ID,
      'script_version': SCRIPT_VERSION,
...
def _assert_attempt_budget(started_monotonic: float) -> None:
  if time.monotonic() - started_monotonic > MAX_WALL_TIME_SECONDS:
    raise RuntimeError('v3.3.4.5 frozen wall-time budget was exceeded.')
  output_size = sum(
      path.stat().st_size for path in OUTPUT_DIR.rglob('*') if path.is_file()
  )
  if output_size > MAX_OUTPUT_BYTES:
    raise RuntimeError('v3.3.4.5 frozen output-storage budget was exceeded.')
...
def _write_nonpublication_terminal(
  ...
  """Publishes the exact v3.3.4.5 ordinary compiler terminal."""
  contract = bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_5
  ...
  if set(record) != set(contract['keys']) or len(record) != 62:
...
def _write_terminal_failure(
  ...
  if (
    ...
    raise RuntimeError('Publication terminal is not the exact 35-key schema.')
...
def _main_v334() -> None:
  if len(sys.argv) != 1:
    raise ValueError('The v3.3.4.5 runner accepts no direct CLI options.')
  ...
  try:
    ...
    _write_common_terminal(
        ...
        stop_reason=None,
        message='All 80 frozen v3.3.4.5 structural sidecar records completed.',
        failure=None,
        results=results,
        failed_current_binding=None,
        compiler=compiler,
