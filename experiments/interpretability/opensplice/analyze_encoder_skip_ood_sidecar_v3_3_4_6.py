#!/usr/bin/env python3
"""CPU-only structural audit for the prospective v3.3.4.6 OOD sidecar.
...
ANALYSIS_VERSION = 'v3.3.4.6-structural-analyzer-v1'
...
INHERITED_V3345_SOURCE_COMMIT = '0da8f47ea6e576a72a1cda204ce868ef79cc2ce5'
...
_FREEZE_PATH = _HERE / 'encoder_skip_ood_sidecar_v3_3_4_6_freeze.json'
_RUN_DIR = _HERE / 'results/v3_3_4_6_development_ood_sidecar_one_shot'
_ANALYSIS_DIR = _HERE / 'results/v3_3_4_6_development_ood_sidecar_analysis'
_ANALYSIS_ATTEMPT_DIR = (
    _HERE / 'results/v3_3_4_6_development_ood_sidecar_analysis_attempt'
)
_PREFLIGHT_DIR = _HERE / 'results/v3_3_4_6_device_preflight'
_PREFLIGHT_CACHE_DIR = _HERE / 'results/v3_3_4_6_preflight_kernel_cache'
_MODEL_CACHE_DIR = _HERE / 'results/v3_3_4_6_model_kernel_cache'
_TEST_PATH = _HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_6_test.py'
...
_V3345_SOURCE_PATHS = (
    'experiments/interpretability/opensplice/analyze_encoder_skip_ood_sidecar_v3_3_4_6.py',
    'experiments/interpretability/opensplice/analyze_encoder_skip_ood_sidecar_v3_3_4_6.sh',
    'experiments/interpretability/opensplice/analyze_encoder_skip_ood_sidecar_v3_3_4_6_test.py',
    'experiments/interpretability/opensplice/generate_encoder_skip_ood_sidecar_v3_3_4_6_freeze.py',
    'experiments/interpretability/opensplice/launch_encoder_skip_ood_sidecar_v3_3_4_6.py',
    'experiments/interpretability/opensplice/run_device_preflight_v3_3_4_6.py',
    'experiments/interpretability/opensplice/run_device_preflight_v3_3_4_6_test.py',
    'experiments/interpretability/opensplice/run_encoder_skip_ood_sidecar_v3_3_4_6.py',
    'experiments/interpretability/opensplice/run_encoder_skip_ood_sidecar_v3_3_4_6.sh',
    'experiments/interpretability/opensplice/run_encoder_skip_ood_sidecar_v3_3_4_6_test.py',
    'experiments/interpretability/opensplice/v3_wider_mechanism/encoder_skip_ood_sidecar_compiler_record_amendment_v3_3_4_6.md',
    'experiments/interpretability/opensplice/validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6.py',
)
...
def _validate_analysis_destination_state(
  ...
  if _ANALYSIS_DIR.exists() or _ANALYSIS_DIR.is_symlink():
    raise AnalysisError('v3.3.4.6 analysis output destination is not fresh.')
  attempt_exists = (
      _ANALYSIS_ATTEMPT_DIR.exists() or _ANALYSIS_ATTEMPT_DIR.is_symlink()
  )
  if active_started_sha256 is None:
    if attempt_exists:
      raise AnalysisError('v3.3.4.6 analysis attempt destination is not fresh.')
    return
  if not _is_sha256(active_started_sha256) or not attempt_exists:
    raise AnalysisError('v3.3.4.6 active analysis attempt is absent.')
  paths = _strict_tree(
  ...
  if (
    ...
    raise AnalysisError('v3.3.4.6 active analysis START changed.')
...
def _validate_atomic_publication_probe(
  ...
  final = _validate_live_publication_file(
      ...
      expected_path='atomic_publication_probe_v3_3_4_6.txt',
  )
...
def _validate_checks(value: Any, anchor: int, label: str) -> None:
  ...
  expected_nonbool = {
      'corrected_host_assertion_version': 'v3.3.4.6',
...
def _validate_record(
  ...
  del allow_invalid  # Invalid current work is represented separately in v3.3.4.6.
  ...
  expected_common = {
      'status': 'complete', 'family': 'v3_3_4_6_unrelated_donor_sidecar_anchor',
...
def _validate_import_file(
  ...
  _exact_keys(value, {
      ...
      'upstream_source_attestation', 'v3_3_4_6_sidecar_sources',
      'created_at_unix_s',
  }, path.name)
  if (
      value.get('schema_version') != 'v3.3.4.6-import-provenance-v1'
  ...
  if value.get('v3_3_4_6_sidecar_sources') != expected_sidecar:
...
def _validate_embedded_consumed_prefix(
  ...
  archive = freeze.get('prior_v3_3_4_5_controlled_stop_archive')
  archive_binding = freeze.get(
      'prior_v3_3_4_5_controlled_stop_archive_content_binding'
  )
  if isinstance(archive, Mapping):
    archive_exact = dict(archive_binding or {}) == _content_binding(archive)
  else:
    archive_exact = (
        isinstance(archive_binding, Mapping)
        and _is_sha256(archive_binding.get('sha256'))
        and isinstance(archive_binding.get('size_bytes'), int)
        and not isinstance(archive_binding.get('size_bytes'), bool)
        and archive_binding['size_bytes'] > 0
    )
  if (
      not archive_exact
      or value.get(
    ...
    raise AnalysisError(
        f'{label} v3.3.4.5 controlled-stop archive binding changed.'
    )
...
def _validate_nonpublication_terminal_contract(value: Any) -> dict[str, Any]:
  ...
  if value != expected:
    raise AnalysisError('Frozen v3.3.4.6 nonpublication contract changed.')
...
def _validate_publication_contract(value: Any) -> dict[str, Any]:
  ...
  if dict(node) != expected:
    raise AnalysisError('Frozen v3.3.4.6 publication contract changed.')
  return dict(node)
...
def _validate_freeze_v3345(
  ...
  if run_dir.resolve() != _RUN_DIR.resolve() or bundle_root.resolve() != _REPO_ROOT:
    raise AnalysisError('v3.3.4.6 production run/repository path changed.')
  ...
  if _sha256(_AMENDMENT_PATH) != AMENDMENT_SHA256:
    raise AnalysisError('v3.3.4.6 amendment bytes changed.')
  ...
  if _git_blob_sha256(AMENDMENT_COMMIT, amendment_relative) != AMENDMENT_SHA256:
    raise AnalysisError('Bound v3.3.4.6 amendment Git blob changed.')
  ...
  freeze = _read_json(_FREEZE_PATH, 'v3.3.4.6 freeze')
  _exact_keys(freeze, _FREEZE_KEYS, 'v3.3.4.6 freeze')
  ...
  import validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6 as bootstrap  # pylint: disable=g-import-not-at-top
  live_v3345_archive = (
      bootstrap.validate_prior_v3_3_4_5_controlled_stop_archive()
  )
  live_v3345_archive_binding = _content_binding(live_v3345_archive)
  if (
      freeze.get('prior_v3_3_4_5_controlled_stop_archive')
      != live_v3345_archive
      or freeze.get(
          'prior_v3_3_4_5_controlled_stop_archive_content_binding'
      ) != live_v3345_archive_binding
  ):
    raise AnalysisError('Frozen v3.3.4.5 controlled-stop archive changed.')
  _validate_publication_contract(freeze.get('publication_contract_v3_3_4_1'))
  _validate_nonpublication_terminal_contract(
      freeze.get('nonpublication_terminal_contract_v3_3_4_6')
  )
  ...
  for key, expected in expected_scalars.items():
    if freeze.get(key) != expected:
      raise AnalysisError(f'v3.3.4.6 freeze.{key} changed.')
  if (
    ...
    raise AnalysisError('v3.3.4.6 freeze scientific order/role contract changed.')
  inventory = freeze.get('file_sha256')
  source_contract = freeze.get('source_inventory_contract')
  if not isinstance(inventory, Mapping) or len(inventory) != 144:
    raise AnalysisError('v3.3.4.6 freeze source inventory is not 144 rows.')
  if not isinstance(source_contract, Mapping):
    raise AnalysisError('v3.3.4.6 source inventory contract is absent.')
  _exact_keys(source_contract, {
      'source_row_count', 'rows', 'prospective_upstream_source_file_count',
      'loaded_scientific_module_contract',
      'inherited_source_authority_commit', 'amendment_authority_commit',
      'implementation_source_authority_commit',
  }, 'freeze.source_inventory_contract')
  rows = source_contract.get('rows')
  implementation_source_commit = source_contract.get(
      'implementation_source_authority_commit'
  )
  if (
      source_contract.get('source_row_count') != 144
      or source_contract.get('prospective_upstream_source_file_count') != 26
      or not isinstance(rows, list) or len(rows) != 144
      or source_contract.get('inherited_source_authority_commit')
      != INHERITED_V3345_SOURCE_COMMIT
      or source_contract.get('amendment_authority_commit') != AMENDMENT_COMMIT
      or not isinstance(implementation_source_commit, str)
      or not re.fullmatch(r'[0-9a-f]{40}', implementation_source_commit)
      or not isinstance(
          source_contract.get('loaded_scientific_module_contract'), list
      )
  ):
    raise AnalysisError('v3.3.4.6 source inventory contract counts changed.')
  row_map = {}
  for raw in rows:
    row = _exact_keys(
        raw, {'path', 'sha256', 'size_bytes', 'git_mode', 'authority_commit'},
  ...
  try:
    ...
    raise AnalysisError('v3.3.4.6 tracked repository is not clean.') from error
  amendment_relative = _AMENDMENT_PATH.relative_to(_REPO_ROOT).as_posix()
  implementation_paths = sorted(
      set(_V3345_SOURCE_PATHS) - {amendment_relative}
  )
  source_parents = subprocess.check_output(
      ('git', '-C', str(bundle_root), 'rev-list', '--parents', '-n', '1',
       implementation_source_commit), text=True,
  ).split()
  launch_parents = subprocess.check_output(
      ('git', '-C', str(bundle_root), 'rev-list', '--parents', '-n', '1', head),
      text=True,
  ).split()
  source_delta = subprocess.check_output(
      ('git', '-C', str(bundle_root), 'diff', '--name-status',
       AMENDMENT_COMMIT, implementation_source_commit), text=True,
  ).splitlines()
  freeze_relative = _FREEZE_PATH.relative_to(_REPO_ROOT).as_posix()
  launch_delta = subprocess.check_output(
      ('git', '-C', str(bundle_root), 'diff', '--name-status',
       implementation_source_commit, head), text=True,
  ).splitlines()
  if (
      source_parents != [implementation_source_commit, AMENDMENT_COMMIT]
      or launch_parents != [head, implementation_source_commit]
      or source_delta != [f'A\t{relative}' for relative in implementation_paths]
      or launch_delta != [f'A\t{freeze_relative}']
  ):
    raise AnalysisError('v3.3.4.6 source/freeze authority graph changed.')
  for relative, digest in inventory.items():
    if not isinstance(relative, str) or not _is_sha256(digest):
      raise AnalysisError('v3.3.4.6 source inventory row is malformed.')
    path = bundle_root / relative
    _strict_regular(path, f'v3.3.4.6 source {relative}')
    mode_line = subprocess.check_output(
    ...
    expected_authority = (
        ...
        else implementation_source_commit
        if relative in implementation_paths
        else INHERITED_V3345_SOURCE_COMMIT
    )
    authority_mode_line = subprocess.check_output(
        ('git', '-C', str(bundle_root), 'ls-tree', expected_authority,
         '--', relative), text=True,
    ).strip()
    if (
        _sha256(path) != digest or _git_blob_sha256(head, relative) != digest
        or _git_blob_sha256(expected_authority, relative) != digest
        or path.stat().st_size != row_map[relative]['size_bytes']
        or git_mode != row_map[relative]['git_mode']
        or not authority_mode_line
        or authority_mode_line.split()[0] != row_map[relative]['git_mode']
        or row_map[relative].get('authority_commit') != expected_authority
    ):
      raise AnalysisError(f'v3.3.4.6 source differs from launch HEAD: {relative}.')
...
def _validate_start_v3345(
  ...
  try:
    ...
    raise AnalysisError('The v3.3.4.6 freeze is not tracked at launch HEAD.') from error
  if (
  ...
  if inventory_attestation != {
      'row_count': 144, 'rows': source_rows,
...
def _validate_preflight_and_same_process(
  ...
  producer_path = _HERE / 'run_device_preflight_v3_3_4_6.py'
  ...
  try:
    producer_tree = ast.parse(
        producer_path.read_text(encoding='utf-8'), filename=str(producer_path)
    )
...
def _validate_manifest_v3345(
  ...
  if (
      manifest.get('schema_version') != 'v3.3.4.6-raw-manifest-v1'
...
def _validate_failed_current(
  ...
  expected = {
      'schema_version': 'v3.3.4.6-failed-current-v1',
...
def _validate_signature_failure(
  ...
  if (
      ...
      != 'v3.3.4.6-program-signature-attestation-v1'
...
def _validate_nonpublication_terminal_archive(
  ...
  if len(node) != 63:
    raise AnalysisError('NONPUBLICATION terminal is not exactly 63 keys.')
  _validate_embedded_consumed_prefix(
  ...
  expected = {
      ...
      'status': 'incomplete_nonpublication_infrastructure_failure',
      'stop_reason': 'post_compile_nonpublication_infrastructure_failure',
      'attempt_id': ATTEMPT_ID, 'script_version': 'v3.3.4.6',
      'amendment_commit': AMENDMENT_COMMIT,
      'amendment_sha256': AMENDMENT_SHA256,
...
def analyze(
  ...
  _assert_cpu_only('v3.3.4.6 analyzer entry')
  ...
  if not run_dir.is_dir() or run_dir.is_symlink():
    raise AnalysisError('v3.3.4.6 run directory is absent or unsafe.')
  ...
  if nonpublication_path.exists():
    ...
    _assert_cpu_only('v3.3.4.6 nonpublication archive exit')
  ...
  if terminal_failure_path.exists():
    ...
    _assert_cpu_only('v3.3.4.6 terminal-failure archive exit')
    return _result_v3345(
  ...
  if post_start_failure.exists():
    ...
    _assert_cpu_only('v3.3.4.6 post-START archive exit')
    return _result_v3345(
        status='complete_controlled_stop_structural_archive',
  ...
  _assert_cpu_only('v3.3.4.6 analyzer exit')
  if full:
...
def _result_v3345(
  ...
  import validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6 as bootstrap  # pylint: disable=g-import-not-at-top
  prior_v3345_archive_binding = _content_binding(
      bootstrap.validate_prior_v3_3_4_5_controlled_stop_archive()
  )
  ...
  result = {
      ...
      'source_and_prior_audit': {
          'current_144_source_rows_exact': True,
          ...
          'prior_v3_3_4_5_controlled_stop_archive_exact': True,
      },
...
def _validate_final_analysis(value: Any) -> dict[str, Any]:
  ...
  if len(node) != 28:
    raise AnalysisError('ANALYSIS does not have the exact 28-key schema.')
  if (
  ...
  source = _exact_keys(node.get('source_and_prior_audit'), {
      'current_144_source_rows_exact', 'historical_96_source_rows_exact',
      ...
      'prior_v3_3_4_5_controlled_stop_archive_exact',
  }, 'ANALYSIS.source_and_prior_audit')
  if len(source) != 14 or any(value is not True for value in source.values()):
...
def render_markdown(result: Mapping[str, Any]) -> str:
  lines = [
      '# OpenSplice v3.3.4.6 OOD sidecar structural audit', '',
...
def _write_outputs(
  ...
  if _ANALYSIS_DIR.exists() or _ANALYSIS_DIR.is_symlink():
    raise FileExistsError('v3.3.4.6 analysis output already exists; never overwrite.')
...
def _create_append_only_directory(path: Path, *, root_role: str) -> None:
  ...
  import validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6 as bootstrap  # pylint: disable=g-import-not-at-top
  _assert_cpu_only('v3.3.4.6 publication-directory helper import')
...
def _publish_new_bytes(
  ...
  """Delegates to the sole frozen v3.3.4.6 publication implementation."""
  ...
  import validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_6 as bootstrap  # pylint: disable=g-import-not-at-top
  _assert_cpu_only('v3.3.4.6 publication helper import')
  ...
  if (
      ...
      != f'.v3345.tmp.{runner_pid}.{ordinal:06d}.{nonce}'
      or any(node.get(key) is not True for key in (
          'file_fsync_before_rename', 'file_fsync_after_fchmod',
          'rename_noreplace_succeeded', 'parent_fsync_succeeded',
          'post_publish_revalidation_exact',
...
def _analysis_attempt_precheck(
  ...
  if (
    ...
    raise FileExistsError('v3.3.4.6 analysis/attempt exists; never resume or retry.')
  ...
  _strict_regular(analyzer_path, 'v3.3.4.6 analyzer')
  _strict_regular(_TEST_PATH, 'v3.3.4.6 analyzer test')
  return {
      'freeze_sha256': freeze_sha,
      'git_head': start['git_head'],
      ...
      'prior_v3_3_4_5_controlled_stop_archive_content_binding': freeze[
          'prior_v3_3_4_5_controlled_stop_archive_content_binding'
      ],
  }
...
def _analysis_started_record(precheck: Mapping[str, Any]) -> dict[str, Any]:
  return {
      ...
      'prior_v3_3_4_5_controlled_stop_archive_content_binding': dict(
          precheck[
              'prior_v3_3_4_5_controlled_stop_archive_content_binding'
          ]
      ),
...
def _validate_active_analysis_attempt(
  ...
  if len(value) != 19:
    raise AnalysisError('ANALYSIS_ATTEMPT_STARTED is not exact 19 keys.')
  expected = {
      'status': 'analysis_attempt_started',
      'analysis_version': ANALYSIS_VERSION,
      'attempt_id': ATTEMPT_ID,
...
def _analysis_failure_record(
  ...
  if len(result) != 20:
    raise AnalysisError('ANALYSIS_FAILURE is not the exact 20-key schema.')
  return result
...
def main() -> None:
  ...
  if not args.acknowledge_structural_only_v3_3_4_6:
