#!/usr/bin/env python3
"""CPU-only contract tests for the v3.3.4.6 OOD sidecar runner."""
...
import run_encoder_skip_ood_sidecar_v3_3_4_6 as runner  # pylint: disable=g-import-not-at-top
...
@functools.cache
def _prior_v3344_prefix_fixture() -> tuple[dict, dict]:
  prefix = runner.bootstrap.validate_prior_v3_3_4_4_consumed_preflight_prefix()
  return prefix, runner.bootstrap.canonical_content_binding(prefix)


@functools.cache
def _prior_v3345_archive_binding_fixture() -> dict:
  archive = (
      runner.bootstrap.validate_prior_v3_3_4_5_controlled_stop_archive()
  )
  return runner.bootstrap.canonical_content_binding(archive)


def _prior_prefix_fields() -> dict:
  prefix, binding = _prior_prefix_fixture()
  v3344_prefix, v3344_binding = _prior_v3344_prefix_fixture()
  return {
      ...
      'prior_v3_3_4_4_consumed_preflight_prefix': copy.deepcopy(v3344_prefix),
      'prior_v3_3_4_4_consumed_preflight_prefix_content_binding': (
          copy.deepcopy(v3344_binding)
      ),
      'prior_v3_3_4_5_controlled_stop_archive_content_binding': (
          copy.deepcopy(_prior_v3345_archive_binding_fixture())
      ),
  }
...
class EncoderSkipOodSidecarV334Test(unittest.TestCase):
  ...
  def test_extraction_nonpublication_terminal_has_exact_six_file_archive(self):
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      self.assertEqual(len(terminal), 63)
      self.assertEqual(terminal['preterminal_tree_binding']['file_count'], 5)
      self.assertEqual(
          {path.relative_to(root).as_posix() for path in root.rglob('*')
           if path.is_file()},
          set(runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_6[
  ...
  def test_postcompile_orchestration_reaches_all_five_exact_stages(self):
    stages = tuple(
        runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_6[
            'failure_stages'
        ]
    )
    for stage in stages:
  ...
  def test_postdiagnostic_record_construction_is_persistence_operation(self):
    lowered = object()
    compiled = object()
    original_v3_3_compiler = {'sentinel': 'original-v3.3-compiler'}
    v3_3_2_compiler = {'sentinel': 'v3.3.2-compiler'}
    wrong_v3_3_3_compiler = {'sentinel': 'wrong-v3.3.3-compiler'}
    ...
    with (
      ...
      result = runner._orchestrate_postcompile_provenance(  # pylint: disable=protected-access
          ...
          launch={
              'v3_3_3_run': {
                  'eight_row_compiler': wrong_v3_3_3_compiler
              },
              'gate_a': {
                  'original_run': {
                      'eight_row_compiler': original_v3_3_compiler
                  },
                  'v3_3_2_run': {
                      'eight_row_compiler': v3_3_2_compiler
                  },
              },
          },
    ...
    self.assertIsNone(result)
    compiler_artifacts.assert_called_once()
    self.assertIs(
        compiler_artifacts.call_args.args[3], original_v3_3_compiler
    )
    self.assertIs(compiler_artifacts.call_args.args[4], v3_3_2_compiler)
  ...
  def test_nonpublication_same_object_rejects_missing_and_extra_keys(self):
    stages = runner.bootstrap.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_6[
        'failure_stages'
    ]
    valid = _same_object()
  ...
  def test_loaded_scientific_module_contract_has_exact_root_roles(self):
    ...
    self.assertTrue({
        'v3_3_4_6_runner', 'v3_3_4_6_launcher',
  ...
  def test_wrapper_and_launcher_freeze_lifecycle(self):
    shell = (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_6.sh').read_text()
    launcher = (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_6.py').read_text()
