#!/usr/bin/env python3
"""Append-only JAX-only device preflight for the v3.3.3 OOD sidecar."""
...
_CACHE_PREIMPORT_ATTESTATION = bootstrap.assert_cache_environment_sanitized()
...
def assert_environment() -> dict[str, str]:
  ...
  live_cache = bootstrap.assert_live_cache_environment_matches(
      _CACHE_PREIMPORT_ATTESTATION
  )
  return {
      **environment,
      'cache_environment': _CACHE_PREIMPORT_ATTESTATION,
...
def validate_freeze_and_bundle() -> dict[str, Any]:
  ...
  expected = {
      ...
      'attempt_id': (
          'opensplice-v3.3.3-development-ood-sidecar-one-shot'
      ),
  ...
  for name, value in expected.items():
    if frozen.get(name) != value:
      raise ValueError(f'v3.3.3 preflight freeze mismatch: {name}.')
  return {
      **binding,
...
def collect_observation() -> dict[str, Any]:
  ...
  observation['v3_3_3_runtime_environment'] = environment
...
def run_preflight() -> tuple[Path, bool]:
  ...
  record = {
      ...
      'external_cache_post_observation': (
          None if observation is None else bootstrap.cache_output_tree_binding(
