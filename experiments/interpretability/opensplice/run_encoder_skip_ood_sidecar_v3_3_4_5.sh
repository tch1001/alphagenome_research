...
if (( $# > 1 )) || { (( $# == 1 )) && [[ "$1" != '--dry-run' ]]; }; then
  echo "v3.3.4.5 accepts no production options; --dry-run is the sole option." >&2
  exit 64
fi
...
while IFS= read -r name; do
  case "$name" in
    ...
    *)
      echo "Unapproved v3.3.4.5 authorization variable: $name" >&2
      exit 64
      ;;
  esac
...
unset LD_LIBRARY_PATH XLA_FLAGS JAX_COMPILATION_CACHE_DIR \
  ...
  ALPHAGENOME_V3_3_4_5_CACHE_ROLE ALPHAGENOME_V3_3_4_5_CACHE_ROOT \
...
exec "$PYTHON_BIN" \
  "$SCRIPT_DIR/launch_encoder_skip_ood_sidecar_v3_3_4_5.py" "$@"
