#!/usr/bin/env python3
"""CPU-only structural audit for the prospective v3.3.4.4 OOD sidecar.
...
ANALYSIS_VERSION = 'v3.3.4.4-structural-analyzer-v1'
...
AMENDMENT_COMMIT = '84c07067613c3a206772c47e59d34b7d49886c7d'
...
PUBLICATION_SCHEMA_VERSION = 'v3.3.4.4-named-temp-renameat2-noreplace-v1'
...
_AMENDMENT_PATH = (
    ...
    / 'encoder_skip_ood_sidecar_preflight_version_amendment_v3_3_4_4.md'
)
...
_FREEZE_PATH = _HERE / 'encoder_skip_ood_sidecar_v3_3_4_4_freeze.json'
_RUN_DIR = _HERE / 'results/v3_3_4_4_development_ood_sidecar_one_shot'
_ANALYSIS_DIR = _HERE / 'results/v3_3_4_4_development_ood_sidecar_analysis'
_ANALYSIS_ATTEMPT_DIR = (
    _HERE / 'results/v3_3_4_4_development_ood_sidecar_analysis_attempt'
)
_PREFLIGHT_DIR = _HERE / 'results/v3_3_4_4_device_preflight'
_PREFLIGHT_CACHE_DIR = _HERE / 'results/v3_3_4_4_preflight_kernel_cache'
_MODEL_CACHE_DIR = _HERE / 'results/v3_3_4_4_model_kernel_cache'
_TEST_PATH = _HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_4_test.py'
...
def _validate_analysis_destination_state(
  ...
  if _ANALYSIS_DIR.exists() or _ANALYSIS_DIR.is_symlink():
    raise AnalysisError('v3.3.4.4 analysis output destination is not fresh.')
  attempt_exists = (
      _ANALYSIS_ATTEMPT_DIR.exists() or _ANALYSIS_ATTEMPT_DIR.is_symlink()
  )
  if active_started_sha256 is None:
    if attempt_exists:
      raise AnalysisError('v3.3.4.4 analysis attempt destination is not fresh.')
    return
  if not _is_sha256(active_started_sha256) or not attempt_exists:
    raise AnalysisError('v3.3.4.4 active analysis attempt is absent.')
  paths = _strict_tree(
  ...
  if (
    ...
    raise AnalysisError('v3.3.4.4 active analysis START changed.')
...
def _validate_atomic_publication_probe(
  ...
  final = _validate_live_publication_file(
      ...
      expected_path='atomic_publication_probe_v3_3_4_4.txt',
  )
  ...
  if (
      ...
      or re.fullmatch(
          rf'\.v3344\.tmp\.{external_pid}\.[0-9]{{6}}\.[0-9a-f]{{32}}',
          collision['path'],
      ) is None
  ):
...
def _validate_record(
  ...
  del allow_invalid  # Invalid current work is represented separately in v3.3.4.4.
  ...
  expected_common = {
      'status': 'complete', 'family': 'v3_3_4_4_unrelated_donor_sidecar_anchor',
...
def _sidecar_source_paths() -> tuple[Path, ...]:
  return tuple(_REPO_ROOT / relative for relative in sorted(_V3344_SOURCE_PATHS))
...
def _validate_import_file(
  ...
  _exact_keys(value, {
      ...
      'upstream_source_attestation', 'v3_3_4_4_sidecar_sources',
      'created_at_unix_s',
  }, path.name)
  if (
      value.get('schema_version') != 'v3.3.4.4-import-provenance-v1'
  ...
  for duplicate_path, names in {
    ...
    if (
        ...
        != 'run_encoder_skip_ood_sidecar_v3_3_4_4.py'
    ):
  ...
  if value.get('v3_3_4_4_sidecar_sources') != expected_sidecar:
...
_FREEZE_KEYS = {
    ...
    'nonpublication_terminal_contract_v3_3_4_4',
    'prior_v3_3_4_3_consumed_preflight_prefix',
}
...
def _validate_embedded_consumed_prefix(
  ...
  if (
    ...
    raise AnalysisError(f'{label} consumed-prefix binding changed.')
...
def _expected_nonpublication_terminal_contract() -> dict[str, Any]:
  ...
  return {
      ...
      'schema_version': 'v3.3.4.4-nonpublication-terminal-v1',
...
def _validate_nonpublication_terminal_contract(value: Any) -> dict[str, Any]:
  ...
  if value != expected:
    raise AnalysisError('Frozen v3.3.4.4 nonpublication contract changed.')
...
def _validate_publication_contract(value: Any) -> dict[str, Any]:
  ...
  expected = {
      ...
      'method': PUBLICATION_METHOD,
      'temp_name_regex': (
          r'^\.v3344\.tmp\.[1-9][0-9]*\.[0-9]{6}\.[0-9a-f]{32}$'
      ),
      'nonce_bytes': 16,
      ...
      'external_preflight_probe_contract': {
          'final_basename': 'atomic_publication_probe_v3_3_4_4.txt',
              'final_sha256': (
                  '47efa8c868d4d9455730ad1e89d6e44afee44172f0d2af7521d8574b7d85ecc9'
          ),
          'final_size_bytes': 49,
  ...
  if dict(node) != expected:
    raise AnalysisError('Frozen v3.3.4.4 publication contract changed.')
  return dict(node)
...
def _validate_freeze_v3344(
  ...
  if run_dir.resolve() != _RUN_DIR.resolve() or bundle_root.resolve() != _REPO_ROOT:
    raise AnalysisError('v3.3.4.4 production run/repository path changed.')
  ...
  if _sha256(_AMENDMENT_PATH) != AMENDMENT_SHA256:
    raise AnalysisError('v3.3.4.4 amendment bytes changed.')
  ...
  if _git_blob_sha256(AMENDMENT_COMMIT, amendment_relative) != AMENDMENT_SHA256:
    raise AnalysisError('Bound v3.3.4.4 amendment Git blob changed.')
  ...
  freeze = _read_json(_FREEZE_PATH, 'v3.3.4.4 freeze')
  _exact_keys(freeze, _FREEZE_KEYS, 'v3.3.4.4 freeze')
  ...
  _validate_nonpublication_terminal_contract(
      freeze.get('nonpublication_terminal_contract_v3_3_4_4')
  )
  ...
  for key, expected in expected_scalars.items():
    if freeze.get(key) != expected:
      raise AnalysisError(f'v3.3.4.4 freeze.{key} changed.')
  if (
    ...
    raise AnalysisError('v3.3.4.4 freeze scientific order/role contract changed.')
  inventory = freeze.get('file_sha256')
  source_contract = freeze.get('source_inventory_contract')
  if not isinstance(inventory, Mapping) or len(inventory) != 120:
    raise AnalysisError('v3.3.4.4 freeze source inventory is not 120 rows.')
  if not isinstance(source_contract, Mapping):
    raise AnalysisError('v3.3.4.4 source inventory contract is absent.')
  ...
  if (
      source_contract.get('source_row_count') != 120
      or source_contract.get('prospective_upstream_source_file_count') != 26
      or not isinstance(rows, list) or len(rows) != 120
      or not isinstance(
          source_contract.get('loaded_scientific_module_contract'), list
      )
  ):
    raise AnalysisError('v3.3.4.4 source inventory contract counts changed.')
  new_shells = {_V3344_SOURCE_PATHS[1], _V3344_SOURCE_PATHS[8]}
  row_map = {}
  for raw in rows:
    row = _exact_keys(
        raw, {'path', 'sha256', 'size_bytes', 'git_mode'},
    ...
    if (
        ...
        or relative in set(_V3344_SOURCE_PATHS) - new_shells
  ...
  try:
    ...
    raise AnalysisError('v3.3.4.4 tracked repository is not clean.') from error
  for relative, digest in inventory.items():
    if not isinstance(relative, str) or not _is_sha256(digest):
      raise AnalysisError('v3.3.4.4 source inventory row is malformed.')
    path = bundle_root / relative
    _strict_regular(path, f'v3.3.4.4 source {relative}')
    mode_line = subprocess.check_output(
    ...
    if (
      ...
      raise AnalysisError(f'v3.3.4.4 source differs from launch HEAD: {relative}.')
...
def _validate_start_v3344(
  ...
  try:
    ...
    raise AnalysisError('The v3.3.4.4 freeze is not tracked at launch HEAD.') from error
  if (
  ...
  if inventory_attestation != {
      'row_count': 120, 'rows': source_rows,
...
def _validate_preflight_and_same_process(
  ...
  producer_path = _HERE / 'run_device_preflight_v3_3_4_4.py'
...
def _validate_manifest_v3344(
  ...
  if (
      manifest.get('schema_version') != 'v3.3.4.4-raw-manifest-v1'
...
def _validate_signature_failure(
  ...
  if (
      ...
      != 'v3.3.4.4-program-signature-attestation-v1'
...
def _validate_nonpublication_terminal_archive(
  ...
  if len(node) != 60:
    raise AnalysisError('NONPUBLICATION terminal is not exactly 60 keys.')
  _validate_embedded_consumed_prefix(
...
def analyze(
  ...
  _assert_cpu_only('v3.3.4.4 analyzer entry')
  ...
  if not run_dir.is_dir() or run_dir.is_symlink():
    raise AnalysisError('v3.3.4.4 run directory is absent or unsafe.')
  freeze, freeze_sha, prior333, original_manifest, _unused, prior331 = (
      _validate_freeze_v3344(
  ...
  if nonpublication_path.exists():
    ...
    _assert_cpu_only('v3.3.4.4 nonpublication archive exit')
    return _result_v3344(
        status='complete_incomplete_nonpublication_infrastructure_archive',
        decision='post_compile_nonpublication_failure_no_scientific_analysis',
        terminal_kind='nonpublication_terminal_failure',
        compiler_state='compiled_without_legal_graph_gate_record',
  ...
  if terminal_failure_path.exists():
    ...
    _assert_cpu_only('v3.3.4.4 terminal-failure archive exit')
    return _result_v3344(
  ...
  if post_start_failure.exists():
    ...
    _assert_cpu_only('v3.3.4.4 post-START archive exit')
    return _result_v3344(
        status='complete_controlled_stop_structural_archive',
  ...
  pairs, failed_binding = _validate_manifest_v3344(
  ...
  compiler = _validate_compiler_v3344(run_dir, completion, freeze, start)
  ...
  _assert_cpu_only('v3.3.4.4 analyzer exit')
  if full:
...
def _result_v3344(
  ...
  result = {
      ...
      'source_and_prior_audit': {
          'current_120_source_rows_exact': True,
...
def _validate_final_analysis(value: Any) -> dict[str, Any]:
  ...
  if len(node) != 25:
    raise AnalysisError('ANALYSIS does not have the exact 25-key schema.')
  if (
  ...
  if len(source) != 12 or any(value is not True for value in source.values()):
...
def render_markdown(result: Mapping[str, Any]) -> str:
  lines = [
      '# OpenSplice v3.3.4.4 OOD sidecar structural audit', '',
...
def _write_outputs(
  ...
  if _ANALYSIS_DIR.exists() or _ANALYSIS_DIR.is_symlink():
    raise FileExistsError('v3.3.4.4 analysis output already exists; never overwrite.')
...
def _create_append_only_directory(path: Path, *, root_role: str) -> None:
  ...
  _assert_cpu_only('v3.3.4.4 publication-directory helper import')
...
def _publish_new_bytes(
  ...
  """Delegates to the sole frozen v3.3.4.4 publication implementation."""
  ...
  import validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_4 as bootstrap  # pylint: disable=g-import-not-at-top
  _assert_cpu_only('v3.3.4.4 publication helper import')
  ...
  if (
      ...
      != f'.v3344.tmp.{runner_pid}.{ordinal:06d}.{nonce}'
      or any(node.get(key) is not True for key in (
          'file_fsync_before_rename', 'file_fsync_after_fchmod',
          'rename_noreplace_succeeded', 'parent_fsync_succeeded',
          'post_publish_revalidation_exact',
...
def _analysis_attempt_precheck(
  ...
  if (
    ...
    raise FileExistsError('v3.3.4.4 analysis/attempt exists; never resume or retry.')
  _assert_predecessor_v334_paths_absent('analysis precheck')
  freeze, freeze_sha, prior333, _manifest, _unused, prior331 = (
      _validate_freeze_v3344(run_dir, bundle_root=bundle_root)
  )
  start = _validate_start_v3344(
      run_dir, freeze, freeze_sha, prior333=prior333, prior331=prior331
  )
  ...
  _strict_regular(analyzer_path, 'v3.3.4.4 analyzer')
  _strict_regular(_TEST_PATH, 'v3.3.4.4 analyzer test')
  return {
      'freeze_sha256': freeze_sha,
      'git_head': start['git_head'],
...
def _analysis_started_record(precheck: Mapping[str, Any]) -> dict[str, Any]:
  return {
      ...
      'acknowledgement': '--acknowledge-structural-only-v3-3-4-4',
...
def main() -> None:
  ...
  parser.add_argument(
      '--acknowledge-structural-only-v3-3-4-4', action='store_true',
      help='Acknowledge the frozen structural-only/no-science boundary.',
  )
  args = parser.parse_args()
  if not args.acknowledge_structural_only_v3_3_4_4:
