#!/usr/bin/env python3
"""CPU-only contract tests for the v3.3.4.6 external preflight."""
...
import launch_encoder_skip_ood_sidecar_v3_3_4_6 as launcher  # pylint: disable=g-import-not-at-top
import run_device_preflight_v3_3_4_6 as preflight  # pylint: disable=g-import-not-at-top
...
def _environment(root: Path, role: str = 'external_preflight') -> dict[str, str]:
  return {
      ...
      'ALPHAGENOME_V3_3_4_6_CACHE_ROLE': role,
      'ALPHAGENOME_V3_3_4_6_CACHE_ROOT': str(root.resolve()),
...
class DevicePreflightV3345Test(unittest.TestCase):
  ...
  def test_three_way_preflight_version_proof_rejects_each_drift(self):
    ...
    wrong_freeze['preflight_script_version'] = 'v3.3.4.6'
    with self.assertRaisesRegex(ValueError, 'three-way proof'):
      module.validate_preflight_version_contract(wrong_freeze)
    with mock.patch.object(module, 'PREFLIGHT_SCRIPT_VERSION', 'v3.3.4.6'):
      with self.assertRaisesRegex(ValueError, 'three-way proof'):
        module.validate_preflight_version_contract(frozen)

    with tempfile.TemporaryDirectory() as directory:
      temporary = Path(directory)
      producer = temporary / 'run_device_preflight_v3_3_4_6.py'
      producer.write_text(
          "PREFLIGHT_SCRIPT_VERSION = 'v3.3.4.6'\n", encoding='utf-8'
      )
      producer_frozen = {
          'preflight_script_version': module.PREFLIGHT_SCRIPT_VERSION,
          'file_sha256': {
  ...
  def test_v3345_controlled_stop_archive_and_copied_binding_are_exact(self):
    module = preflight.bootstrap
    archive = module.validate_prior_v3_3_4_5_controlled_stop_archive()
    binding = module.canonical_content_binding(archive)
    self.assertEqual(
        set(archive['model_attempt']['six_root_filesystem_state']),
        {
            'model_run', 'device_preflight', 'external_cache', 'model_cache',
            'analysis_attempt', 'analysis_output',
        },
    )
    self.assertEqual(
        archive['model_attempt']['status'],
        'controlled_stop_diagnostic_provenance_failure',
    )
    self.assertEqual(
        archive['structural_archive']['status'],
        'complete_controlled_stop_structural_archive',
    )
    with mock.patch.object(
        module, 'validate_prior_v3_3_4_5_controlled_stop_archive',
        return_value=archive,
    ):
      self.assertEqual(
          module.validate_recorded_prior_v3_3_4_5_controlled_stop_archive(
              binding
          ), archive,
      )
      for changed in (
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      freeze_record = {
          ...
          'prior_v3_3_4_5_controlled_stop_archive_content_binding': (
              preflight.bootstrap.canonical_content_binding(
                  preflight.bootstrap
  ...
  def test_launcher_order_validates_before_model_cache_and_start(self):
    ...
    tokens = (
        'path_absence = bootstrap.validate_gate_a_path_absence()',
        "run_device_preflight_v3_3_4_6.py'), '--run'",
  ...
  def test_launcher_dry_cache_is_nonproduction_and_removed(self):
    ...
    try:
      self.assertTrue(root.name.startswith('alphagenome-v3.3.4.6-dry-cache.'))
  ...
  def test_revised_serializers_all_bind_consumed_prefix_and_content(self):
    ...
    keysets = (
        ...
        module.NONPUBLICATION_TERMINAL_CONTRACT_V3_3_4_6['keys'],
    )
    self.assertEqual([len(value) for value in keysets], [23, 39, 25, 69, 36, 63])
    for keyset in keysets:
      self.assertTrue(required <= set(keyset))

  def test_freeze_generator_is_deterministic_exact_88_144_144(self):
    generator = importlib.import_module(
        'generate_encoder_skip_ood_sidecar_v3_3_4_6_freeze'
    )
    ...
    self.assertIn('nonpublication_terminal_contract_v3_3_4_6', first)
    self.assertNotIn('nonpublication_terminal_contract_v3_3_4_5', first)
    predecessor = json.loads(
        preflight.bootstrap.V3_3_4_5_FREEZE_PATH.read_text(encoding='utf-8')
    )
