#!/usr/bin/env python3
"""Standard-library-only pre-import gate for the v3.3.4.3 OOD sidecar."""
...
FREEZE_PATH = _HERE / 'encoder_skip_ood_sidecar_v3_3_4_3_freeze.json'
OUTPUT_DIR = (
...
PREFLIGHT_DIR = _HERE / 'results' / 'v3_3_4_3_device_preflight'
PREFLIGHT_KERNEL_CACHE_DIR = (
    _HERE / 'results' / 'v3_3_4_3_preflight_kernel_cache'
)
MODEL_KERNEL_CACHE_DIR = _HERE / 'results' / 'v3_3_4_3_model_kernel_cache'
...
def _next_publication_identity() -> tuple[int, str, str]:
  ...
  temporary = f'.v3343.tmp.{os.getpid()}.{ordinal:06d}.{nonce}'
...
CACHE_ROLE_ENVIRONMENT = 'ALPHAGENOME_V3_3_4_3_CACHE_ROLE'
CACHE_ROOT_ENVIRONMENT = 'ALPHAGENOME_V3_3_4_3_CACHE_ROOT'
...
AUTHORIZED_GIT_HEAD_ENV = 'V3343_AUTHORIZED_GIT_HEAD'
AUTHORIZED_FREEZE_SHA256_ENV = 'V3343_AUTHORIZED_FREEZE_SHA256'
AUTHORIZED_FREEZE_SIZE_ENV = 'V3343_AUTHORIZED_FREEZE_SIZE_BYTES'
...
RAW_RECORD_CONTRACT = {
    ...
    'family': 'v3_3_4_3_unrelated_donor_sidecar_anchor',
    'top_level_keys': [
...
def assert_cache_environment_sanitized() -> dict[str, Any]:
  ...
  if role not in ('external_preflight', 'model', 'dry_run') or not root_text:
    raise ValueError('A frozen v3.3.4.3 cache role/root is required.')
  ...
  if role == 'dry_run' and not root.name.startswith(
      'alphagenome-v3.3.4.3-dry-cache.'
  ):
...
def validate_one_shot_output_absence(
  ...
  for path, label in (
      (output_dir, 'v3.3.4.3 sidecar output'),
      (analysis_dir, 'v3.3.4.3 sidecar analysis'),
      (analysis_attempt_dir, 'v3.3.4.3 sidecar analysis attempt'),
  ):
    _reject_confirmation_path(path)
...
def validate_predecessor_path_absence() -> dict[str, Any]:
  """Requires both never-launched predecessor path sets to remain absent."""
  result = {}
  for version, paths in (
      ('v3_3_4', V3_3_4_PREDECESSOR_PATHS),
      ('v3_3_4_1', V3_3_4_1_PREDECESSOR_PATHS),
...
def validate_preflight_state_for_role() -> dict[str, Any]:
  ...
  expected = {
      'script_version': 'opensplice-device-preflight-v3.3.4.3',
      'status': 'pass',
...
def _validate_started_output_prefix() -> dict[str, Any]:
  ...
  for path, label in (
      (ANALYSIS_DIR, 'v3.3.4.3 analysis output'),
      (ANALYSIS_ATTEMPT_DIR, 'v3.3.4.3 analysis attempt'),
  ):
    if path.exists() or path.is_symlink():
...
def validate_freeze(*, allow_started_output: bool = False) -> dict[str, Any]:
  for path in (
    ...
    _reject_confirmation_path(path)
  _validate_file(AMENDMENT_PATH, AMENDMENT_SHA256, 'v3.3.4.3 amendment')
  _validate_file(
  ...
  for name, value in expected.items():
    if frozen.get(name) != value:
      raise ValueError(f'v3.3.4.3 freeze mismatch: {name}.')
  if len(frozen) != 84 or len(frozen.get('file_sha256', {})) != 108:
    raise ValueError('v3.3.4.3 freeze key/source counts changed.')
  ...
  if (
      inventory_contract.get('source_row_count') != 108
  ...
  if not isinstance(source_rows, list) or len(source_rows) != 108:
    raise ValueError('Frozen 108-row source inventory changed.')
  ...
  if not isinstance(file_sha, dict) or not file_sha:
    raise ValueError('v3.3.4.3 frozen file inventory is absent.')
  for relative, expected_sha in sorted(file_sha.items()):
    path = (_REPO / relative).resolve()
    ...
    _validate_file(path, expected_sha, f'v3.3.4.3 bundle {relative}')
  ...
  if subprocess.check_output(
    ...
    raise ValueError('v3.3.4.3 requires globally tracked-clean HEAD before import.')
