#!/usr/bin/env python3
"""CPU-only contract tests for the v3.3.4.3 OOD sidecar runner."""
...
import run_encoder_skip_ood_sidecar_v3_3_4_3 as runner  # pylint: disable=g-import-not-at-top
...
class EncoderSkipOodSidecarV334Test(unittest.TestCase):
  ...
  def test_extraction_nonpublication_terminal_has_exact_six_file_archive(self):
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      self.assertEqual(len(terminal), 58)
  ...
  def test_loaded_scientific_module_contract_has_exact_root_roles(self):
    ...
    self.assertTrue({
        'v3_3_4_3_runner', 'v3_3_4_3_launcher',
  ...
  def test_wrapper_and_launcher_freeze_lifecycle(self):
    shell = (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_3.sh').read_text()
    launcher = (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_3.py').read_text()
