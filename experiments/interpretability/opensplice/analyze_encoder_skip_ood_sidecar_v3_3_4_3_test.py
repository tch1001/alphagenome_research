#!/usr/bin/env python3
"""CPU-only tests for the standalone v3.3.4.3 structural analyzer."""
...
_ANALYZER_PATH = _HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_3.py'
...
def _dispatch_event(
  ...
  value: dict[str, object] = {
      'schema_version': 'v3.3.4.3-dispatch-event-v1',
...
class StructuralResultAndAppendOnlyTest(unittest.TestCase):
  ...
  def test_structural_result_draft_has_literal_23_keys_and_no_science(self):
    result = analyzer._result_v3343(  # pylint: disable=protected-access
  ...
  def test_shell_accepts_only_literal_acknowledgement(self):
    shell = (_HERE / 'analyze_encoder_skip_ood_sidecar_v3_3_4_3.sh').read_text(encoding='utf-8')
    self.assertIn('--acknowledge-structural-only-v3-3-4-3', shell)
...
class PublicationAmendmentTest(unittest.TestCase):
  ...
  def _publication_terminal_fixture(
    ...
    temp_relative = (
        'compiler/eight_row/'
        f'.v3343.tmp.{runner_pid}.000000.{"a" * 32}'
    )
  ...
  def test_publication_contract_is_literal_and_exact(self):
    ...
    bootstrap_path = (
        _HERE / 'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_3.py'
    )
    ...
    preflight_source = (
        _HERE / 'run_device_preflight_v3_3_4_3.py'
  ...
  def test_output_state_represents_all_publication_failure_classes(self):
    for case in ('temporary', 'uncertain', 'preexisting'):
      with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
        ...
        audit = _publication_audit()
        if case == 'temporary':
          path = output / f'.v3343.tmp.12.000001.{"a" * 32}'
        else:
    ...
    with tempfile.TemporaryDirectory() as directory:
      ...
      def publish_bytes(role, relative, payload, *, artifact_role):
        ...
        return {
            ...
            'temp_basename': f'.v3343.tmp.{__import__("os").getpid()}.000000.{nonce}',
      ...
      with mock.patch.dict(
          sys.modules,
          {'validate_encoder_skip_ood_sidecar_bootstrap_v3_3_4_3': fake},
      ):
...
class NonpublicationTerminalAmendmentTest(unittest.TestCase):
  ...
  def test_exact_58_key_contract_is_literal(self):
    self.assertEqual(len(analyzer.NONPUBLICATION_TERMINAL_KEYS), 58)
    self.assertEqual(len(set(analyzer.NONPUBLICATION_TERMINAL_KEYS)), 58)
    self.assertIn(
  ...
  def test_runner_identity_literals_match_committed_protocol(self):
    runner = ast.parse(
        (_HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_3.py').read_text(
            encoding='utf-8'
        )
    )
    values = {}
  ...
  def test_runner_diagnostic_classifier_is_operation_typed(self):
    runner_path = _HERE / 'run_encoder_skip_ood_sidecar_v3_3_4_3.py'
  ...
  def test_machine_extracted_serializer_keysets_match_analyzer(self):
    ...
    launcher = ast.parse(
        (_HERE / 'launch_encoder_skip_ood_sidecar_v3_3_4_3.py').read_text(
            encoding='utf-8'
        )
    )
    runner = ast.parse(
    ...
    self.assertEqual(
        assigned_dict_keys(
            function(runner, '_write_common_terminal'), 'record', 64
        ),
        analyzer._RUN_COMPLETE_KEYS,  # pylint: disable=protected-access
    )
    self.assertEqual(
        assigned_dict_keys(
            function(runner, '_write_terminal_failure'), 'record', 31
        ),
        analyzer.TERMINAL_FAILURE_KEYS,
    )
    self.assertEqual(
        assigned_dict_keys(
            function(runner, '_write_nonpublication_terminal'), 'record', 58
        ),
        analyzer.NONPUBLICATION_TERMINAL_KEYS,
    )
    self.assertEqual(len(analyzer._FREEZE_KEYS), 84)  # pylint: disable=protected-access
    self.assertEqual(len(analyzer._V3343_SOURCE_PATHS), 12)  # pylint: disable=protected-access
  ...
  def test_nonpublication_result_is_exact_structural_only_outcome(self):
    ...
    self.assertEqual(
        result['analysis_version'], 'v3.3.4.3-structural-analyzer-v1'
    )
  ...
  def test_analyze_routes_nonpublication_without_raw_access(self):
    with tempfile.TemporaryDirectory() as directory:
      ...
      with (
          mock.patch.object(
              analyzer, '_validate_freeze_v3343',
              return_value=({}, 'b' * 64, {'prior': 333}, {}, {}, {'prior': 331}),
          ),
          mock.patch.object(analyzer, '_validate_start_v3343', return_value=start),
          mock.patch.object(
